...
class BAFLineDP(nn.Module):
    def __init__(self, embed_dim, gru_hidden_dim, gru_num_layers, bafn_output_dim, dropout, device, 
                 use_static_features=False, static_feature_dim=32):
        ...
        self.gru = nn.GRU(
            actual_embed_dim, gru_hidden_dim, num_layers=gru_num_layers, 
            batch_first=True, bidirectional=True, dropout=dropout
        )

        # define BAFN module
        self.bafn = weight_norm(
            BAFN(l_dim=actual_embed_dim, c_dim=2 * gru_hidden_dim, h_dim=bafn_output_dim, h_out=2),
            name='q_mat', dim=None
        )
    ...
    def forward(self, code_tensor, static_features=None):
        ...
        if self.use_static_features and static_features is not None:
            ...
            if static_features.size(1) != code_tensor.size(1):
                ...
                if static_features.size(1) < max_len:
                    ...
                    static_features = torch.cat([static_features, padding], dim=1)
                if code_tensor.size(1) < max_len:
                    pad_size = max_len - code_tensor.size(1)
                    padding = torch.zeros(code_tensor.size(0), pad_size, code_tensor.size(2), 
