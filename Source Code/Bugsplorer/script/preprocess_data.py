...
split_dir = os.path.join(current_script_dir, "../dataset/linedp/splits/")
...
def make_splits():
    ...
    # 只处理activemq项目
    releases = all_releases["activemq"]

    # 训练集: 前3个版本
    train_releases = releases[:3]
    # 验证集: 第4个版本
    val_releases = [releases[3]]
    # 测试集: 最后一个版本
    test_releases = [releases[4]]
    ...
    train_releases = [
        releases[0]  # 第一个版本作为训练集
        for releases in all_releases.values()
    ]
    val_releases = [
        releases[1]  # 第二个版本作为验证集
        for releases in all_releases.values()
    ]
    test_releases = [
        release
        for releases in all_releases.values()
        for release in releases[2:]  # 第三个及以后的版本作为测试集
    ]
