...
class BugPredictorTrainer(ModelLoader):
    ...
    def __init__(self):
        ...
        train_dataset, val_dataset, test_dataset = dataset_manager.load_dataset()
    ...
    def start(self):
        ...
        self.info(f"Training completed in {timedelta(seconds=end_time-start_time)}")
    ...
    def train(self):
        ...
        for curr_epoch in count(1):
            ...
            for step_i, _ in enumerate(epoch_progress_bar):
                global_step += 1
                batch = next(cyclic_data_loader)

                if len(batch) == 5:
                    source_tensor, label_tensor, _, _, handcrafted_tensor = batch
                elif len(batch) == 4:
                    source_tensor, label_tensor, _, _ = batch
                    handcrafted_tensor = None
                elif len(batch) == 3:
                ...
                loss, logits = self.model(
                    ...
                    label_tensor.float().to(self.device),  
                    handcrafted_tensor=handcrafted_tensor.to(self.device) if handcrafted_tensor is not None else None
                )
                ...
                avg_training_loss = training_loss / (step_i + 1)
            ...
            self.info(
                ...
                f"Precision={validation_score.precision_score:2.3f}, "
                f"F1={validation_score.f1_score:2.3f}, "
                f"R@20%={validation_score.recall_at_top20_percent_loc:2.3f}, "
                f"E@20%={validation_score.effort_at_top20_percent_recall:2.3f}, "
            ...
            score = (
                ...
                + validation_score.recall_at_top20_percent_loc / 0.8
                + (1 - validation_score.effort_at_top20_percent_recall) / 0.9
            )
