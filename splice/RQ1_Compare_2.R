...
save.fig.dir = './RQ1_result2/RQ1_figure2/'
...
get.line.metrics.result = function(baseline.df, cur.df.file)
{
  ...
  # Calculate AUC for each file - ensure same order as IFA
  auc_df <- sorted %>% group_by(filename) %>% 
    summarize(auc = tryCatch({
      if(sum(line.level.ground.truth == "True") > 0 && sum(line.level.ground.truth == "False") > 0) {
        roc_obj <- roc(line.level.ground.truth, -order, quiet = TRUE)
  ...
  # Merge AUC with IFA to ensure same files
  ifa_with_auc <- merge(data.frame(filename = IFA$filename), auc_df, by = "filename", all.x = TRUE)
  auc.list <- ifa_with_auc$auc
  
  result.df = data.frame(IFA$filename, ifa.list, recall.list, effort.list, auc.list, IFA$totalSLOC)
...
# Calculate AUC for each method - ensure same files as IFA
AUC_df = sorted %>% merge(total_true) %>% group_by(test, filename) %>% 
  summarize(auc = tryCatch({
    if(sum(line.level.ground.truth == "True") > 0 && sum(line.level.ground.truth == "False") > 0) {
      roc_obj <- roc(line.level.ground.truth, -order, quiet = TRUE)
      as.numeric(auc(roc_obj))
  ...
  arrange(test, filename)

AUC_SPLICE_S_df = sorted_SPLICE_S %>% merge(total_true) %>% group_by(test, filename) %>% 
  summarize(auc = tryCatch({
    if(sum(line.level.ground.truth == "True") > 0 && sum(line.level.ground.truth == "False") > 0) {
      roc_obj <- roc(line.level.ground.truth, -order, quiet = TRUE)
      as.numeric(auc(roc_obj))
...
AUC_SPLICE_G_df = sorted_SPLICE_G %>% merge(total_true) %>% group_by(test, filename) %>% 
  summarize(auc = tryCatch({
    if(sum(line.level.ground.truth == "True") > 0 && sum(line.level.ground.truth == "False") > 0) {
      roc_obj <- roc(line.level.ground.truth, -order, quiet = TRUE)
      as.numeric(auc(roc_obj))
...
AUC_SPLICE_F_df = sorted_SPLICE_F %>% merge(total_true) %>% group_by(test, filename) %>% 
  summarize(auc = tryCatch({
    if(sum(line.level.ground.truth == "True") > 0 && sum(line.level.ground.truth == "False") > 0) {
      roc_obj <- roc(line.level.ground.truth, -order, quiet = TRUE)
      as.numeric(auc(roc_obj))
...
# Merge AUC with IFA data to ensure same files
AUC_merged = merge(data.frame(test = IFA$test, filename = IFA$filename), AUC_df, by = c("test", "filename"), all.x = TRUE)
AUC_SPLICE_S_merged = merge(data.frame(test = IFA_SPLICE_S$test, filename = IFA_SPLICE_S$filename), AUC_SPLICE_S_df, by = c("test", "filename"), all.x = TRUE)
AUC_SPLICE_G_merged = merge(data.frame(test = IFA_SPLICE_G$test, filename = IFA_SPLICE_G$filename), AUC_SPLICE_G_df, by = c("test", "filename"), all.x = TRUE)
AUC_SPLICE_F_merged = merge(data.frame(test = IFA_SPLICE_F$test, filename = IFA_SPLICE_F$filename), AUC_SPLICE_F_df, by = c("test", "filename"), all.x = TRUE)

## prepare data for plotting
deeplinedp.ifa = IFA$order - 1
deeplinedp.recall = recall20LOC$recall20LOC
deeplinedp.effort = effort20Recall$effort20Recall
deeplinedp.auc = AUC_merged$auc

deepline.dp.line.result = data.frame(IFA$filename, deeplinedp.ifa, deeplinedp.recall, deeplinedp.effort, deeplinedp.auc, IFA$test, IFA$totalSLOC)
...
splice_S.auc = AUC_SPLICE_S_merged$auc

splice_S.line.result = data.frame(IFA_SPLICE_S$filename, splice_S.ifa, splice_S.recall, splice_S.effort, splice_S.auc, IFA_SPLICE_S$test, IFA_SPLICE_S$totalSLOC)
...
splice_G.auc = AUC_SPLICE_G_merged$auc

splice_G.line.result = data.frame(IFA_SPLICE_G$filename, splice_G.ifa, splice_G.recall, splice_G.effort, splice_G.auc, IFA_SPLICE_G$test, IFA_SPLICE_G$totalSLOC)
...
splice_F.auc = AUC_SPLICE_F_merged$auc

splice_F.line.result = data.frame(IFA_SPLICE_F$filename, splice_F.ifa, splice_F.recall, splice_F.effort, splice_F.auc, IFA_SPLICE_F$test, IFA_SPLICE_F$totalSLOC)

sum_glance.lr.result.df = glance.lr.result.df %>%   summarize(IFA=median(ifa.list), recall20=median(recall.list), effort20=median(effort.list), AUC=median(auc.list, na.rm=TRUE), .by=test)
sum_glance.ea.result.df = glance.ea.result.df %>%   summarize(IFA=median(ifa.list), recall20=median(recall.list), effort20=median(effort.list), AUC=median(auc.list, na.rm=TRUE), .by=test)
sum_glance.md.result.df = glance.md.result.df %>%   summarize(IFA=median(ifa.list), recall20=median(recall.list), effort20=median(effort.list), AUC=median(auc.list, na.rm=TRUE), .by=test)
sum_deepline.dp.line.result = deepline.dp.line.result %>%   summarize(IFA=median(deeplinedp.ifa), recall20=median(deeplinedp.recall), effort20=median(deeplinedp.effort), AUC=median(deeplinedp.auc, na.rm=TRUE), .by=IFA.test)
sum_linedp.result.df = linedp.result.df %>% summarise(IFA=median(ifa.list), recall20=median(recall.list), effort20=median(effort.list), AUC=median(auc.list, na.rm=TRUE), .by=test)
sum_splice_S.line.result = splice_S.line.result %>%   summarize(IFA=median(splice_S.ifa), recall20=median(splice_S.recall), effort20=median(splice_S.effort), AUC=median(splice_S.auc, na.rm=TRUE), .by=IFA_SPLICE_S.test)
sum_splice_G.line.result = splice_G.line.result %>%   summarize(IFA=median(splice_G.ifa), recall20=median(splice_G.recall), effort20=median(splice_G.effort), AUC=median(splice_G.auc, na.rm=TRUE), .by=IFA_SPLICE_G.test)
sum_splice_F.line.result = splice_F.line.result %>%   summarize(IFA=median(splice_F.ifa), recall20=median(splice_F.recall), effort20=median(splice_F.effort), AUC=median(splice_F.auc, na.rm=TRUE), .by=IFA_SPLICE_F.test)

names(sum_glance.lr.result.df) = c("release", "IFA", "recall20", "effort20", "AUC")
names(sum_glance.ea.result.df) = c("release", "IFA", "recall20", "effort20", "AUC")
names(sum_glance.md.result.df)  = c("release", "IFA", "recall20", "effort20", "AUC")
names(sum_linedp.result.df) = c("release", "IFA", "recall20", "effort20", "AUC")
names(sum_deepline.dp.line.result) = c("release", "IFA", "recall20", "effort20", "AUC")
names(sum_splice_S.line.result) = c("release", "IFA", "recall20", "effort20", "AUC")
names(sum_splice_G.line.result) = c("release", "IFA", "recall20", "effort20", "AUC")
names(sum_splice_F.line.result) = c("release", "IFA", "recall20", "effort20", "AUC")

#### get data results of models ####
write.csv(sum_splice_S.line.result, file = './RQ1_result2/SPLICE-S.csv')
write.csv(sum_splice_G.line.result, file = './RQ1_result2/SPLICE-G.csv')
write.csv(sum_splice_F.line.result, file = './RQ1_result2/SPLICE-F.csv')
write.csv(sum_glance.lr.result.df, file = './RQ1_result2/GLANCE-LR.csv')
write.csv(sum_glance.ea.result.df, file = './RQ1_result2/GLANCE-EA.csv')
write.csv(sum_glance.md.result.df, file = './RQ1_result2/GLANCE-MD.csv')
write.csv(sum_linedp.result.df, file = './RQ1_result2/LineDP.csv')
write.csv(sum_deepline.dp.line.result, file = './RQ1_result2/DeepLineDP.csv')
...
# all.line.result = rbind(sum_glance.lr.result.df, sum_glance.ea.result.df, sum_glance.md.result.df, sum_linedp.result.df, sum_deepline.dp.line.result, sum_splice_S.line.result, sum_splice_G.line.result, sum_splice_F.line.result)
all.line.result = rbind(
  sum_deepline.dp.line.result %>% mutate(model='DeepLineDP'),
  sum_splice_S.line.result %>% mutate(model='SPLICE-S'),
  sum_splice_G.line.result %>% mutate(model='SPLICE-G'),
  sum_splice_F.line.result %>% mutate(model='SPLICE-F')
)

recall.result.df = select(all.line.result, c('technique', 'recall20'))
ifa.result.df = select(all.line.result, c('technique', 'IFA'))
effort.result.df = select(all.line.result, c('technique', 'effort20'))
auc.result.df = select(all.line.result, c('technique', 'AUC'))
...
auc.result.df = preprocess(auc.result.df, FALSE)
...
ggplot(auc.result.df, aes(x=reorder(variable, -value, FUN=median), y=value)) + geom_boxplot() +
  stat_summary(fun = mean, geom = "point", shape = 17, size = 1.5, color = "red", position = position_dodge(0.75)) + facet_grid(~rank, drop=TRUE, scales = "free", space = "free") + ylab("AUC") + xlab("") + theme(axis.text.x=element_text(angle = -60, hjust = 0))
ggsave(paste0(save.fig.dir, "file-AUC.pdf"),width=7,height=2.5)